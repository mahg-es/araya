...
- **Authority:** Professor=STRATEGIC (final). Manu=WHAT (Product). Aurora=WHO CAN (Capability). Sonia=HOW (Delivery). Daneel=COORDINATE (Relay Controller — routes to the specialist bench per task, never functional owner, `can_write_code=false`). Clara=TEST_AUTOMATION (writes/executes tests, Relay EXECUTING). Teresa=TEST_GATE (independent gate, Relay TESTING, binding PASS/FAIL). Rolando=REALITY_AUTHORITY (reports to The Data Professor; binding VERIFIED/DISCREPANCY). Giskard=**retired** (2026-07-20) — non-operational; historical references only; any operational reference = Relay T-030 BLOCK.
...
- **Multi-project coordination model (canonical, 2026-07-19; updated 2026-07-26):** one terminal/session per project, each anchored in that project's own repo, `AGENTS.md`, and PostOffice channel — never one terminal invoking multiple projects. Daneel is the single cross-project coordinator (Relay Controller), working through each project's file-based PostOffice (`.araya/postoffice/`), not by operating inside each project's terminal. The Professor runs the human side of each project's terminal when he chooses to advance it; Daneel reviews/dispatches/escalates via PostOffice in between. When Daneel needs the Professor's direct attention on something, it must name the exact terminal/project to switch to, explicitly — never leave it implicit. *(Supersedes the 2026-07-19 version, which named Giskard — now retired, non-operational — as coordinator.)*
